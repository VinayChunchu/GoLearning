...
func (user User) GetStatus() {
	fmt.Println("Status of the user is : ", user.Status)
}

// When structs are passed to methods, a copy is passed NOT the actual struct.
// This is another case where we use pointers
func (user User) NewMail() {
	user.Email = "chand@VinayChunchu.com"
	fmt.Println("New email of the user: ", user.Email)
}
